 ...
 *   Generated on: 2026-08-24 02:52:12
 *   PAP version: 5.16.0.1, GPF version: 1.3.70.0
 *   
 */


define('PAP_API_VERSION', '1.0.0.6');
...
if (!class_exists('Gpf_Rpc_Array', false)) {
  class Gpf_Rpc_Array extends Gpf_Object implements Gpf_Rpc_Serializable, IteratorAggregate {
  	...
  	public function toText(): string {
  		return (string) json_encode($this->toObject(), JSON_PRETTY_PRINT);
  	}
...
if (!class_exists('Gpf_Rpc_Server', false)) {
  class Gpf_Rpc_Server extends Gpf_Object {
      ...
      // Must match MultiRequest::MAX_BATCH_SIZE — the java enforces the same limit.
      ...
      private function setDecoder($request) {
          if ($this->isFormRequest($request)) {
              if (!Gpf_Log_LoggerDisplay::shouldEnableDisplayLogger()) {
      ...
      public function run(Gpf_Rpc_Params $params) {
          ...
          $requestArray = $this->getRequestsArray($params);
          if (count($requestArray) > self::MAX_BATCH_SIZE) {
              throw new Gpf_Exception(sprintf('Batch size limit %d exceeded', self::MAX_BATCH_SIZE));
          }
       ...
       * @service
       */
      public function syncTime(Gpf_Rpc_Params $params): Gpf_Rpc_Action {
          ...
          $timeOffset /= self::ONE_SECOND_IN_MILLISECONDS;
          if (abs($timeOffset) > self::ONE_DAY_IN_SECONDS) {
              $timeOffset = (int)$timeOffset % self::ONE_DAY_IN_SECONDS;
          }
  ...
  class Gpf_Http extends Gpf_Object implements Gpf_HttpResponse {
      ...
      public function setHeaderValue($name, $value, $replace = true, $httpResponseCode = 0) {
          ...
          if(headers_sent($fileName, $line)) {
              Gpf_Log::serverLogWarning("Headers already sent in $fileName line $line while setting header $name: $value");
              throw new Gpf_Exception("Headers already sent while setting header $name");
          }
      ...
      public static function getRemoteIpFull($onlyRemote = false, ?array $serverParams = null) {
          ...
          if (isset($serverParams['HTTP_X_FORWARDED_FOR'])) {
              $ip = self::parseXForwardedForIp($serverParams['HTTP_X_FORWARDED_FOR']);
              if ($ip != '') {
                  return $ip;
              }
          }
...
if (!class_exists('Gpf_Data_Filter', false)) {
  class Gpf_Data_Filter extends Gpf_Object implements Gpf_Rpc_Serializable {
      ...
      const STARTS = 'L%';
      const NOT_STARTS = 'NL%';
      public const FILE_UPLOADED = 'FE';
      public const FILE_NOT_UPLOADED = 'FNE';
...
if (!class_exists('Gpf_Rpc_GridRequest', false)) {
  class Gpf_Rpc_GridRequest extends Gpf_Rpc_Request {
      ...
      /** @psalm-suppress PossiblyUnusedParam used in customer's API scripts */
...
if (!class_exists('Gpf_Data_RecordSet', false)) {
  class Gpf_Data_RecordSet extends Gpf_Object implements IteratorAggregate, Gpf_Rpc_Serializable {
       ...
       */
      public function add(array|Gpf_Data_Row $record): void {
...
if (!class_exists('Gpf_Net_Http_Request', false)) {
  class Gpf_Net_Http_Request extends Gpf_Object {
      ...
      public function getUserAgent(): string {
          return ($this->userAgent === '') ? self::getDefaultUserAgent() : $this->userAgent;
      }
      ...
      public static function getDefaultUserAgent(): string {
          ...
          return 'PostAffiliateProApi/1.0';
      }
  	...
  	private function parseUrl() {
  		...
  		if ($components === false) {
              Gpf_Log::serverLogWarning('Gpf_Net_Http_Request: Failed to parse malformed URL: ' . $this->url);
  			throw new Gpf_Exception('Failed to parse malformed URL: ' . $this->url);
  		}
...
if (!class_exists('Gpf_Rpc_FilterCollection', false)) {
  class Gpf_Rpc_FilterCollection extends Gpf_Object implements IteratorAggregate {
      ...
      public function matches(Gpf_Data_Row $row): bool {
          foreach ($this->filters as $filter) {
...
if (!class_exists('Gpf_Rpc_Action', false)) {
  class Gpf_Rpc_Action extends Gpf_Object implements Gpf_Rpc_Serializable {
      ...
      public function appendErrorMessage(string $message): void {
          $this->errorMessage = trim($this->errorMessage . ' ' . $message);
      }
...
if (!class_exists('Gpf_Log', false)) {
  class Gpf_Log {
      ...
      private static function serverLog(string $message, int $errorLevel): void {
          ...
          if ($hostedAccountId === null) {
              if (defined('UNIT_TEST_ENV')) {
                  return;
              }
              // multi-tenant infrastructure: tenant id is provided per request by the proxy layer
              $hostedAccountId = Gpf_DbEngine_Configuration_TenantIdProvider::getTenantIdFromHeader() ?? 'unknown';
          }
       ...
       *      Gpf_Log::DEBUG
       */
      public static function addLogger($type, $logLevel): void {
          $type instanceof Gpf_Log_LoggerBase
              ? self::getLogger()->addLogger($type, $logLevel)
              : self::getLogger()->add($type, $logLevel);
      }
...
if (!class_exists('Gpf_Log_Logger', false)) {
  class Gpf_Log_Logger extends Gpf_Object {
      ...
      public function add($type, $logLevel) {
          $this->addLogger($this->create($type), $logLevel);
      }
       ...
       * @param int $logLevel
       */
      public function addLogger(Gpf_Log_LoggerBase $logger, $logLevel): void {
          if(!$this->checkLoggerTypeExists($logger->getType())) {
          	$logger->setLogLevel($logLevel);
          	$this->loggers[$logger->getType()] = $logger;
  
              if (Gpf_Log_LoggerDisplay::shouldEnableDisplayLogger() &&
                  $logger->getType() != Gpf_Log_LoggerDisplay::TYPE &&
                  !$this->checkLoggerTypeExists(Gpf_Log_LoggerDisplay::TYPE)) {
                  $displayLogger = new Gpf_Log_LoggerDisplay($logger->getType());
                  $displayLogger->setLogLevel(Gpf_Log::DEBUG);
                  $this->loggers[Gpf_Log_LoggerDisplay::TYPE] = $displayLogger;
              }
          } else {
  ...
  class Pap_Tracking_Request extends Gpf_Object {
      ...
      public function toString() {
          ...
          foreach($this->request as $key => $value) {
              $params .= ($params != '' ? ", " : '').Gpf_Common_String::convertToUtf8($key).'='.Gpf_Common_String::convertToUtf8(var_export($value, true));
          }
          return $params;
      }
